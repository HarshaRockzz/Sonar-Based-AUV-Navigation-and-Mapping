...
class MainWindow(QMainWindow):
    ...
    def update_graphs(self):
     amplitude = 0.00005  # Adjust amplitude as needed
     frequency = 5  # Adjust frequency as needed
     interpolation_factor = 5  # Number of points to interpolate between main points
     ...
     for field in self.graphs:
        ...
        if field == "Latitude vs Longitude":
            x_data = np.array(self.data["Longitude"])
            y_data = np.array(self.data["Latitude"])

            # Ensure data is not empty
            if len(x_data) == 0 or len(y_data) == 0:
                print("Longitude or Latitude data is empty.")
                continue

            x_interpolator = interp1d(range(len(x_data)), x_data, kind='linear')
            y_interpolator = interp1d(range(len(y_data)), y_data, kind='linear')

            interp_indices = np.linspace(0, len(x_data) - 1, len(x_data) * interpolation_factor)
            x_data_interp = x_interpolator(interp_indices)
            y_data_interp = y_interpolator(interp_indices)

            # Calculate wave-modified path
            wave_latitudes = [lat + amplitude * np.sin(frequency * i) for i, lat in enumerate(y_data_interp)]
            wave_longitudes = [lon + amplitude * np.cos(frequency * i) for i, lon in enumerate(x_data_interp)]

            # Plot Accurate Path with interpolated points
            ax.plot(x_data_interp, y_data_interp, '-o', color='#ff7f0e', markersize=3, linewidth=1.5, label="Accurate Path")

            # Plot Wave-Modified Path with interpolated points
            ax.plot(wave_longitudes, wave_latitudes, '-o', color='blue', markersize=3, linewidth=1.5, label="Wave-Modified Path")

            # Set axis labels, title, and limits based on data
            ax.set_xlabel("Longitude")
            ax.set_ylabel("Latitude")
            ax.set_title("Latitude vs Longitude")
            ax.set_xlim(min(x_data_interp), max(x_data_interp))
            ax.set_ylim(min(y_data_interp), max(y_data_interp))
            ax.legend(facecolor='#151B54', labelcolor='white')
            ax.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.5)

            # Load the AUV image
            auv_img = Image.open("C:/Users/mamid/OneDrive/Desktop/Data Transfer/auv-800.jpg")
            imagebox = OffsetImage(auv_img, zoom=0.05)  # Adjust zoom level if needed
            ab = AnnotationBbox(imagebox, (x_data_interp[0], y_data_interp[0]), frameon=False)
            ax.add_artist(ab)

            # Update function for the animation
            def update(frame):
                ab.xy = (x_data_interp[frame], y_data_interp[frame])
                return ab,

            # Animate the AUV along the Accurate Path
            ani = FuncAnimation(
                canvas.figure, update, frames=len(x_data_interp), interval=200, blit=False, repeat=False
            )

        else:
            # Plot other fields over time
...
class Map3DWindow(QMainWindow):
    ...
    # Add markers on both paths at every 10th point
     marker_interval = 5  # Place markers every 10th point
    ...
    def update_map(self):
        ...
        if len(latitudes) > 0 and len(latitudes) == len(longitudes):
            ...
            # Adjust the wave starting position to match the original path's starting point
            wave_latitudes[0] = latitudes[0]
            wave_longitudes[0] = longitudes[0]
